...
portal,target:pchar;
task:pointer;
buffer,data:array [0..512-1] of byte;
returned_lba:int64;
ptr:dword;
pv:pointer;
iov:tscsi_iovec ;
...
if task<>nil
  ...
  //for i:=0 to sizeof(buffer)-1 do memo1.Lines.Add(inttostr(buffer[i]));
  if buffer[0]=0 then ;
  scsi_free_scsi_task(task);
  end
  else memo1.Lines.Add(strpas(iscsi_get_error(iscsi)));
//
...
if task=nil then
  ...
  memo1.Lines.Add('msg:'+strpas(iscsi_get_error(iscsi)));
  end
  else
  begin
  memo1.Lines.Add('status:'+inttostr(Pscsi_task(task)^.status));
...
if task=nil then
  ...
  scsi_free_scsi_task(task);
  if Pscsi_task(task)^.status<>0 then exit;
  task:=nil;
  end;
inc(pos,size);
